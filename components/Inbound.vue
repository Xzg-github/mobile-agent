...
<script>
	...
	export default {
		...
		props: ['stra','strb','strc','contentTime','endTime','contentState','phoneCallId','meetingFlag'],
		data() {
			return {
				phone: '', //电话号码
				province: '', //省份
		...
		methods: {
			...
			call(){
				var self = this
				api.getDialNumber(self.strb,self.uerInfo.areaCode,self.apiDomian,self.uerInfo.token,function(data){
					...
					uni.getStorage({
						...
						success: function(res) {
							if (!res.data) {
								...
								this.$emit("getPageLen",1);  // 关闭呼入弹窗 显示首页2个按钮
							}
						}
					})
				}else {
			...
			history_date(){
				...
				var self = this
				api.getCusInfo(self.apiDomian,self.uerInfo.token,self.strb,self.starDate,self.endDate,function(data){
					if(data[1].data.data){
			...
			getBasicInfo(){
				...
				var self = this
				api.customerList(self.apiDomian,self.uerInfo.token,self.strb,function(data){
					if(!data[1].data.data){
						this.cus_info = ''
					}else{
			...
			formSubmit() {
				...
				//存储服务小结的内容
				var post_data = {
					callType: 1, //呼叫方向 0呼出 1呼入
					connectStatus: this.contentState == '' ? 'N':'Y', //联系状态，Y-接通；N-未接通，后期需要软电话接口判断
					id: this.id, //名单ID（在外呼名单接口提取）
				...
				//联络历史更新
				var data = {
					serviceResult1: self.picketData[0][self.multiIndex[0]].name,
					serviceType1: self.picketData[0][self.multiIndex[0]].id,
